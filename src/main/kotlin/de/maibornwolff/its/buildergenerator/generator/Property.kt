...
data class Property(
    ...
    private fun generateDefaultForComplexType(
        project: Project,
        config: GeneratorConfig
    ) =
        generateDefaultFromBuilder(project, config) ?: CodeBlock.of("TODO(\"Needs·a·default·value!\")")
    ...
    private fun generateDefaultFromBuilder(
        project: Project,
        config: GeneratorConfig
    ): CodeBlock? {
    ...
    companion object {
        ...
        private val collectionsDefaultValuesMap = mapOf(
            "List" to "emptyList()",
            "Map" to "emptyMap()",
            "Set" to "emptySet()",
            "Array" to "emptyArray()"
